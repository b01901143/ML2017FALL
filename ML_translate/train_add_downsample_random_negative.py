...
ans=[]
rm1=np.random.randint(45036, size=(45036*10,2))
for i in range((45036*10)):
    train_x.append(train_text[rm1[i,0]])
    if(train_text[rm1[i,0]]==train_text[rm1[i,1]]):
        ans.append(1)
    else:
        ans.append(0)
rm2=np.random.randint(45036, size=(45036*10,2))
for i in range((45036*10)):
...
model.fit([train_ap, padded_docs], ans, validation_split=0.0, batch_size=512, epochs=40, callbacks=[chechpoint],verbose=1)
#model.fit([train_ap, padded_docs], ans, validation_split=0.2, epochs=30, callbacks=[chechpoint],verbose=1)

model.save('m_dlstm_2:10_full_100.hdf5')
'''
# test
