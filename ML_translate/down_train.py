...
train_ap=np.concatenate((train_down,train_ap0,train_ap0,train_ap0,train_ap0,train_ap0,train_ap0),axis=0)
# input data
train_text=[]
...
ans=[1]*2*len(train_text)+[0]*5*len(train_text)
ans=np.array(ans)
...
model.fit([train_ap, padded_docs], ans, validation_split=0.2, batch_size=512, epochs=30, callbacks=[chechpoint],verbose=1)
#model.fit([train_ap, padded_docs], ans, validation_split=0.2, epochs=30, callbacks=[chechpoint],verbose=1)

model.save('m_down_sample_60.hdf5')
'''
# test
