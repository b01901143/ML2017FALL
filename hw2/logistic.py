...
def train(X_all, Y_all, save_dir):
    ...
    for epoch in range(1, epoch_num):
        ...
        if (epoch) % save_param_iter == 0:
            ...
            print('epoch avg acc = %f' % (acc.sum() / (float(save_param_iter) * train_data_size)))
            total_loss = 0.0
            acc = np.zeros(batch_size)
        ...
        for idx in range(step_num):
            ...
            w_grad = np.sum(-1 * X * (np.squeeze(Y) - y).reshape((batch_size,1)), axis=0) + lambda_w*w
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str,
                        default='logistic_output/', dest='output_path',
                        help='Path to save the model parameters')
